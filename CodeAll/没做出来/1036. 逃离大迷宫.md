...
在一个 106 x 106 的网格中，每个网格上方格的坐标为 `(x, y)` 。现在从源方格 `source = [sx, sy]` 开始出发，意图赶往目标方格 `target = [tx, ty]` 。数组 `blocked` 是封锁的方格列表，其中每个 `blocked[i] = [xi, yi]` 表示坐标为 `(xi, yi)` 的方格是禁止通行的。
...
- 我以为这题是巧妙解答,结果标签告诉我是bfs+hash表,但是数据范围又不允许我这么写
- 看了评论才知道原来是blocked有长度范围,它最多只能挡住一定面积或者一定数量的点,再往后就挡不住了,又把visited 给忘了,bfs一定要visited!!!

```java
class Solution {
    public boolean isEscapePossible(int[][] blocked, int[] source, int[] target) {
        Map<String,Boolean> blockpos = new HashMap<>();
        for(int[] block:blocked){
            blockpos.put(""+block[0]+"+"+block[1],true);
        }
        return bfs(blockpos,source,target)&&bfs(blockpos,target,source);
    }
    public boolean bfs(Map<String,Boolean> blockpos,int[] source,int[] target){
        ...
        while(!dq.isEmpty()){
            ...
            for(int k=0;k<4;k++){
                int x = ax[0]+direct[k];
                int y = ax[1]+direct[k+1];
                if(x==target[0]&&y==target[1]) return true;
                if(x>=0&&y>=0&&x<1000000&&y<1000000&&!(blockpos.getOrDefault(""+x+"+"+y,false)))
                    if(visited.add(""+x+"+"+y))
                        dq.offer(new int[]{x,y});
                if(dq.size()>200) return true;
            }
        }
        return false;
    }
